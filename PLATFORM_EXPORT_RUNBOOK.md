...
不想记命令？**把下载好的「品类文件夹」直接拖到项目里的图标上**：

```
双筛-把下载文件夹拖到这里.bat
```

它会全自动完成：找文件 → 整理 → 解析 → ERP图搜 → 模型精筛 → 出老板版报告，跑完**自动打开报告文件夹**。你全程不用输入任何东西（品类名自动从文件夹名取，内部 slug 自动生成）。

> 建议把这个 `.bat` 发送快捷方式到桌面，以后下载完直接拖过去即可。
> 等价的一条命令（终端里跑）：
> ```powershell
> python -X utf8 -m sourcing.cli platform-export-run --src "D:\IXSPY下载数据\男女内衣及家居服"
> ```
> 小样本先试：加 `--limit 30`。

下面是**手动分步版**（排查问题或想控制每一步时用）：
